...
function Header({ onMenuClick, sidebarCollapsed, onToggleSidebar }) {
    ...
    useEffect(() => {
        ...
        // 🔥 if connects later (most important)
        socket.on("connect", requestOnlineCount);

        return () => {
            socket.off("online-count");
            socket.off("connect", requestOnlineCount);
        };
    }, []);
            ...
            <div className="h-full px-6 flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 rounded-[14px] backdrop-blur-[20px] bg-gradient-to-r from-red-400/20 to-orange-400/20 border border-red-300/40 hover:from-red-400/30 hover:to-orange-400/30 transition-all active:scale-95 group"
                    >
                        <LogOut className="w-4 h-4 text-red-700 group-hover:-translate-x-0.5 transition-transform" />
                        <span className="hidden sm:inline text-sm font-bold text-red-700">Logout</span>
                    </button>
                </div>
            </div>
        </header>
