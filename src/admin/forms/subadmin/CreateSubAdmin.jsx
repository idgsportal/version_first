...
function CreateSubAdmin() {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!formData.shopName || !formData.subAdminName || !formData.mobile) {
        ...
        <div className="space-y-6 p-3">
            ...
            <div className="backdrop-blur-[24px] bg-white/60 rounded-[24px] border border-white/40 shadow-[0_8px_24px_rgba(0,0,0,0.06)] p-6">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-gray-800">Create Sub-Admin</h2>
                            <p className="text-sm text-gray-600">Add new sub-admin to the system</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="backdrop-blur-[24px] bg-white/60 rounded-[26px] border border-white/40 shadow-[0_8px_24px_rgba(0,0,0,0.06)] p-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        <label className="block text-gray-700 mb-2">Sub-Admin Name *</label>
                        <input
                            type="text"
                ...
                {/* File Uploads */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    {['Shop Photo', 'Profile Photo', 'Aadhaar Card', 'PAN Card'].map((label) => (
                        <div key={label}>
                            <label className="block text-gray-700 mb-2">Upload {label} *</label>
                            <input type="file" className="hidden" id={`subadmin-${label.replace(' ', '-').toLowerCase()}`} />
                            <label
                                htmlFor={`subadmin-${label.replace(' ', '-').toLowerCase()}`}
                ...
                <div className="mb-6 p-6 rounded-[20px] bg-gradient-to-br from-purple-400/10 to-pink-400/10 border border-purple-300/30">
                    <h3 className="text-gray-800 mb-4">Service Access Control</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {Object.entries(serviceAccess).map(([key, value]) => (
                            <div key={key} className="flex items-center justify-between p-3 rounded-[14px] bg-white/40">
                                <span className="text-sm text-gray-800 capitalize">{key.replace(/([A-Z])/g, ' $1').trim()} Service</span>
                                <button
                                    type="button"
                                    onClick={() => setServiceAccess({ ...serviceAccess, [key]: !value })}
